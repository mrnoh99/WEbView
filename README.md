...
- **Safari 스타일 크롬**: 뒤로/앞으로, 새로고침/정지, 로딩 진행 표시줄, 전체화면, 공유
- **제스처**: 가장자리 스와이프로 뒤로/앞으로 이동
- **슬라이드 편집(부가 기능)**: 연 문서가 reveal.js 슬라이드면 뷰어 상단의 **‘편집’**
  버튼이 나타나 PowerPoint 스타일로 슬라이드를 편집·저장할 수 있습니다.
  일반 HTML 에는 편집 버튼이 표시되지 않습니다.

> 동작 흐름: **뷰어가 기본**입니다. 파일을 열면 항상 Safari 처럼 먼저 표시되고,
> 편집 가능한 문서일 때만 ‘편집’ 버튼으로 에디터에 진입합니다. 에디터에서 ‘뷰어’를
>누르면 (저장한 내용이 반영된 채) 다시 뷰어로 돌아갑니다.
