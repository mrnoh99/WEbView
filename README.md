...
## 주요 기능
- **파일 열기**: 시스템 문서 선택기로 어디에 저장된 HTML 이든 선택해서 열기
- **다른 앱에서 열기**: Files 앱 등에서 공유 → `HTML 뷰어`, 또는 "다음으로 열기" 지원
  (`CFBundleDocumentTypes` + `LSSupportsOpeningDocumentsInPlace`)
- **상대경로 리소스 로딩**: HTML 과 같은 폴더(및 하위 폴더)의 CSS·JS·이미지를 함께 로드
  (`loadFileURL(_:allowingReadAccessTo:)` 로 폴더 읽기 권한 부여)
- **Safari 스타일 크롬**: 뒤로/앞으로, 새로고침/정지, 로딩 진행 표시줄, 공유
- **최근 파일**: 보안 스코프 북마크로 저장 — 앱을 다시 켜도 같은 파일에 재접근
